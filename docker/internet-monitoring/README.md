...
## Pre-requisites
Before we get started installing the Prometheus stack. Ensure you install the latest version of docker and [docker-compose](https://docs.docker.com/compose/install/) on your Docker host machine. This has been tested with Docker for Mac, Synology, and RPi and it works.
...
You will also need to change variables in the `config.monitoring.example` file. Follow the intructions in the comments and then save as `config.monitoring` and this will load login and notification information into Grafana.
...
username - admin
password - changeme! (Password is the one stored in the `config.monitoring` env file)
